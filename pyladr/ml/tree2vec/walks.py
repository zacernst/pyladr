"""Tree walk generation for Tree2Vec embedding training.
...
suitable for skip-gram training.
"""
...
class TreeWalker:
    ...
    def _depth_first(
        ...
        tok = _node_token(
            term, position, depth,
            self.config.include_position, self.config.include_depth,
            var_id_map,
        )
        result = [tok]
    ...
    def _path_walks(self, term: Term) -> list[list[str]]:
        ...
        return paths if paths else [[_node_token(
            term, 0, 0, self.config.include_position, self.config.include_depth,
            {} if self.config.include_var_identity else None,
        )]]
