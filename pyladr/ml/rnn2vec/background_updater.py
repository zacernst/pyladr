"""Background worker for online RNN2Vec model updates.

Runs rnn2vec.update_online() on a daemon thread so the proof search loop
...
----------------------
RNN2Vec.update_online() runs gradient steps on the encoder. The encoder
is set back to eval mode after each update. BackgroundRNN2VecUpdater
calls provider._rnn2vec.update_online() directly.
...
class BackgroundRNN2VecUpdater:
    """Runs RNN2Vec update_online() on a daemon thread.

    Args:
        provider: The RNN2VecEmbeddingProvider whose _rnn2vec model is
    ...
    def submit(self, batch: list[Clause]) -> bool:
        ...
        try:
            ...
            logger.debug("BackgroundRNN2VecUpdater: queue full, batch dropped")
            return False
    ...
    def _worker(self) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            try:
                ...
                logger.warning("BackgroundRNN2VecUpdater: update failed", exc_info=True)
            finally:
                self._queue.task_done()

    def _run_update(self, batch: list[Clause]) -> None:
        provider = self._provider
        # update_online runs gradient steps then sets encoder back to eval
        stats = provider._rnn2vec.update_online(batch, learning_rate=self._lr)
        ...
        logger.info(
            "BackgroundRNN2VecUpdater: update #%d done — pairs=%d, loss=%.4f, model_v=%d",
            update_num,
        ...
        if self._completion_callback is not None:
            try:
                ...
                logger.warning(
                    "BackgroundRNN2VecUpdater: completion callback failed", exc_info=True
                )
